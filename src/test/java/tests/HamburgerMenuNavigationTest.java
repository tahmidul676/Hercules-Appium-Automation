...
/**
 * Navigates: Dashboard -> Hamburger -> [User Profile, Leave Management, Sr
 * Schedule Plan, Trade Promotion, Sync Now] (each opened & backed out of) ->
 * Logout.
 *
 * Driver/service lifecycle (start Appium server, launch app, quit driver, stop
 * service) is handled entirely by AndroidBase's @BeforeClass/@AfterClass — this
 * class just consumes the inherited `driver` and builds its own `wait` around
 * it.
 *
...
public class HamburgerMenuNavigationTest extends AndroidBase {
	...
	// NOTE: The original @text='User ID'/@hint='User ID' xpath did not match
	// anything on
	// this device/app build (see NoSuchElementException in the last run) — hint
	// text on an
	// empty EditText isn't always exposed via @text or @hint depending on the
	// Android/
	// UiAutomator2 version and whether the field is native vs. Compose. Falling
	// back to
	// positional locators (1st/2nd EditText on the login screen) which matches the
	// two
	// visible fields in the screenshot regardless of how the hint is exposed.
	// Once you have Appium Inspector output, replace these with resource-id
	// locators, e.g.:
	// By.id("com.sslwireless.dsas:id/etUserId") /
	// By.id("com.sslwireless.dsas:id/etPin")
	...
	// The single content-desc-based locator kept failing to match anything real on
	// this
	// app build (see repeated NoSuchElementException across runs). Since we don't
	// yet have
	// an Appium Inspector / page-source dump to confirm the real attribute, this
	// tries several
	...
	private final By[] hamburgerIconCandidates = {
			...
			By.xpath(
					"//*[contains(translate(@content-desc,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'menu')]"),
			By.xpath(
					"//*[contains(translate(@content-desc,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'navigate up')]"),
			By.xpath(
					"//*[contains(translate(@content-desc,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'drawer')]"),
			// resource-id naming conventions
			By.xpath(
					"//*[contains(@resource-id,'menu') or contains(@resource-id,'hamburger') or contains(@resource-id,'drawer') or contains(@resource-id,'nav_icon') or contains(@resource-id,'toolbar_icon')]"),
			// common Android widget classes for a toolbar icon button, positioned first in
			// the hierarchy
			By.xpath("(//android.widget.ImageButton)[1]"), By.xpath("(//android.widget.ImageView)[1]") };
	private final By userProfileRow = By
			.xpath("//*[contains(@text,'8801313006854') or contains(@text,'H M Shahjalal Shanto')]");

	// ---- User Profile ("Update Profile") page fields ----
	// Confirmed from the actual screen layout (screenshot): fields appear in this
	// fixed
	// order — Profile Name (English), Profile Name (বাংলা), Phone number, Email,
	// Address —
	// all android.widget.EditText, followed by an "Update" button.
	// NOTE: the earlier "following::" xpath approach caused an Appium/UiAutomator2
	// XPath
	// engine crash (ArrayList$ListItr cannot be cast to NodeType) — this is a known
	// bug with
	// that axis combined with translate(), not a locator-correctness issue.
	// Positional
	// locators avoid it entirely and are safe here since the field order is now
	// confirmed.
	...
	// A generic "did the sub-page actually load" check — adjust per page if you
	// have
	// a stable header/title element on each screen instead of relying on menu
	// disappearing.
	...
	private void performLogin(String userId, String password) {
		...
		// Confirm login succeeded by waiting for the post-login dashboard's "Sync Now"
		// button
		// to appear — the hamburger icon isn't guaranteed to render this early, so we
		// don't
		// wait on it here. A longer timeout is used since this involves a network round
		// trip.
		new WebDriverWait(driver, Duration.ofSeconds(30)).until(ExpectedConditions.visibilityOfElementLocated(syncNow));
	}

	public void clickSyncNow() {
		// Stage 1: dashboard's "Sync Now" button — this navigates into a sync listing
		// page,
		...
		// if the app's flow changes and it's not there, don't fail the whole test over
		// it.
		try {
		...
		// Sync itself may take a while over the network. Instead of waiting on one
		// guessed
	...
	/**
	 * Locates the hamburger icon. On the first call, tries each candidate locator
	 * in turn and caches whichever one actually matches — every subsequent call
	 * (i.e. every time we return to the home page after a module navigation) reuses
	 * that same cached locator directly instead of re-scanning the candidate list,
	 * so we always target the same real element rather than risking a different
	 * positional fallback winning on a different screen.
	 */
	...
	private void openHamburgerMenu() {
		...
		for (int attempt = 1; attempt <= maxAttempts; attempt++) {
			try {
				...
				// Covers StaleElementReferenceException, TimeoutException (overlay never
				// showed),
				// and similar — re-locate and try the click again rather than failing
				// immediately.
				lastError = e;
				System.out.println("openHamburgerMenu attempt " + attempt + " failed: " + e.getClass().getSimpleName()
						+ " — retrying...");
			}
		}
		throw lastError;
	 ...
	 * Field order (Profile Name EN, Profile Name BN, Phone, Email, Address) is
	 * confirmed from the actual "Update Profile" screen layout, so positional
	 * locators are used directly.
	 */
	private void updateUserProfile(String email, String address) {
		...
		// Update may navigate back to the dashboard on its own. If it doesn't, this
		// ensures
	...
	private void clickAndVerifyThenBack(By locator, String pageName) {
		...
		// + pageName + "']")));
		// Assert.assertTrue(header.isDisplayed(), pageName + " page did not load as
		// expected");
	...
	private void navigateBack() {
		// Prefer the device back key over driver.navigate().back() for native Android
		// apps —
	...
	private void performLogout() {
		...
		// If a confirmation dialog ("Are you sure you want to logout?") appears, handle
		// it here.
